...
(If you think you can do these things by extending `sp` then I'm very interested to see.)
...
## here each part becomes an object, and each object only has one part
## (ignoring hole-vs-island status for now)
wone <- spFromTable(sptable(oz)  %>% filter(y < -39.5) %>% mutate(object = branch),   crs = proj4string(oz))
```
