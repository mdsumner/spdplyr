...
test_that("group by and summarize is quiet", {
  ...
  expect_s4_class(g[1:20, ] %>% summarize(alon = mean(LON), mxlat = max(LAT), mxarea = max(AREA)),
                   "SpatialPolygonsDataFrame")
  expect_s4_class(wrld_simpl[1:20, ]  %>% group_by(REGION, SUBREGION) %>% summarize(alon = mean(LON), mxlat = max(LAT), mxarea = max(AREA)),
                "SpatialPolygonsDataFrame")
  expect_s4_class(g[1:10, ] %>% summarise(alon = mean(LON)), "SpatialPolygonsDataFrame")
